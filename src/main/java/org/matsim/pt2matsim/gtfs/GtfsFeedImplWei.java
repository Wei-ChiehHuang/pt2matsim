 ...
 * @author polettif
 */
public class GtfsFeedImplWei implements GtfsFeed {

	protected static final Logger log = Logger.getLogger(GtfsFeedImplWei.class);
	...
	public GtfsFeedImplWei(String gtfsFolder) {
	...
	protected void loadRoutes() throws IOException {
		...
		try {
			...
			while(line != null) {
				...
				//Todo: agencyId is added and RouteImplWei is used to process flixbus data
				...
				Route newGtfsRoute = new RouteImplWei(routeId, shortName, longName, extendedRouteType, agencyId);
		...
		try {
			...
			while(line != null) {
				...
				if(route == null) {
					...
					// store Trip
					//Todo: change to RouteImplWei for handling flixbus
					((RouteImplWei) route).addTrip(newTrip);
